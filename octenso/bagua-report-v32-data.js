...
(function(){
'use strict';
// 零術語白話短語:lamp=「這個特質在做什麼」(鍵名沿用 lamp,不改動呼叫端);short=punch 句用的極短語
// 2026-07-27 Simon 定案:文案不用「燈」的比喻、也不強調「這一個月」——改用特質/狀態語彙(較貼近心理學用語)
var PLAIN={
...
var OPEN={
 contrast:function(a,b){ return {
   story:'你身上最突出的特質，是「'+a.lamp+'」；而最安靜的角落，是「'+b.lamp+'」。一邊常常出場，一邊幾乎沒有戲份，強處與空處之間，像一道斷崖。',
   punch:'所以這份快照想先說的，只有一句：你不缺'+a.short+'，你缺的可能是'+b.short+'。'
 };},
 peak:function(list){
   var story = (list.length>=2)
     ? '你身上最突出的特質有兩個：「'+list[0].lamp+'」和「'+list[1].lamp+'」。它們輪流出場、也常常同台——你的日子，多半是被這兩股力氣推著走的。'
     : '你身上有一個特別突出的特質：「'+list[0].lamp+'」。多數時候，都是它在前面帶路。';
   return {story:story, punch:'這份快照想先說的是：你最突出的地方很清楚——值得想的，是別讓其他特質，一直待在暗處。'};
 },
 level:function(band){
   if(band==='high') return {story:'你的各種特質幾乎都在線上——每一種力氣都在場上，沒有明顯偏廢，也沒有明顯空白。',punch:'這份快照想先說的是：你不缺火力，值得看顧的是電量。'};
   if(band==='low') return {story:'你的各種特質此刻都比較安靜——不是沒有力氣，比較像整體在省電、在蓄能。',punch:'這份快照想先說的是：安靜不等於空，可能只是還沒到它們出場的時候。'};
   return {story:'你的特質有強有弱，但沒有哪一個特別搶眼——遇到什麼場面，就用哪一種，換得很自然。',punch:'這份快照想先說的是：你的本錢不在單點，而在換檔。'};
 }
};
