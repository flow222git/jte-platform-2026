...
var MAX_ENTRIES=300, MAX_TEXT=8000, MAX_CTX=12000;
...
function add(o){
  ...
  var ctx=String((o&&o.context)||'').trim();
  if(ctx.length>MAX_CTX) ctx=ctx.slice(0,MAX_CTX)+'…(超長截斷)';
  var list=load();
  ...
  if(ctx) e.context=ctx; // 脈絡=當次的卦象/解讀紀錄;個人八態鏡引用的仍是 text(本人的字)
  list.push(e);
...
/* 收錄 opt-in 區塊。opts={source,label,getText,getContext,viewerHref}
   getText/getContext 在按下當下才呼叫(拿到最新的感想欄與 AI 解讀)。
   text=本人寫下的字(日後八態鏡引用來源);context=當次卦象/解讀紀錄(留完整脈絡)。 */
function mountOptIn(container,opts){
  ...
  var desc=opts.getContext
    ?'收你自己寫下的字(問題與感想),連同這次的卦象與解讀紀錄,存在這台裝置、不上傳;之後可用八態鏡照出跨次的個人規律。隨時可看、可刪。'
    :'只收你自己寫下的字(問題與感想),存在這台裝置、不上傳;之後可用八態鏡照出跨次的個人規律。隨時可看、可刪。';
  box.innerHTML=
    '<div class="oc-t" style="margin-bottom:2px">這段話,收進你的八態素材嗎?</div>'
    +'<div style="font-size:12px;opacity:.72;margin-bottom:10px">'+desc+'</div>'
  ...
  box.querySelector('.oc-yes').onclick=function(){
    ...
    var c=(opts.getContext&&opts.getContext())||'';
    var r=add({source:opts.source,label:opts.label,text:t,context:c});
