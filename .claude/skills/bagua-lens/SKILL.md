...
5. **正式收斂測試**(使用者要求時):派兩個獨立 subagent 盲判(prompt 一字不差、互不知對方、只准讀 schema+素材)。**指標算法不在此複寫**,依 schema 之 `convergence` 區塊(states-schema=八態 presence 一致率+引文重疊;lens-schema=六格一致率)。三件事缺一不可:算出數字 → 記進 `octenso/CALIBRATION.md`(含 Simon 的達標判定與一句理由)→ 分歧回填混淆對照。**跑過不等於測過**:沒有算出數字的輪次不計入升版之路;先聲明這一輪測的是哪一層,兩層不可互充。
