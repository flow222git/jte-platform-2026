...
def main():
    ...
    meta = doc.get("meta", {})
    check("meta.version == 0.3.3-draft", str(meta.get("version")) == "0.3.3-draft", f"got {meta.get('version')!r}")
    ...
    jl = gp.get("判讀紀律", {})
    check("判讀紀律六鍵+次型原則", all(x in jl for x in ["證據純化律", "同爻雙訊號裁決", "成爻門檻", "主體同一性律", "爻疑氾濫規則", "素材次型註記", "次型原則"]))
    check("爻疑在位", "爻疑" in str(jl.get("同爻雙訊號裁決", "")))
    check("孤證不成爻", "孤證" in str(jl.get("成爻門檻", "")))
    check("主體同一性:同一時段", "同一時段" in str(jl.get("主體同一性律", "")))
    check("爻疑氾濫:拒判", "拒判" in str(jl.get("爻疑氾濫規則", "")))
    check("次型五型", isinstance(jl.get("素材次型註記"), list) and len(jl["素材次型註記"]) == 5)
    ap = doc.get("applicability", {})
    check("適用域宣告在位", bool(ap))
    check("適用域三條件", "單一運作主體" in str(ap.get("原則", "")) and "同一時段" in str(ap.get("原則", "")))
    check("純文本不判", "不判" in str(ap.get("純文本不判", "")))
    check("語用檢查:拒判", "拒判" in str(ap.get("語用檢查", "")))
    gk = gp.get("歸格例句庫", [])
