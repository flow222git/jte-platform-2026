...
// meta 與誠實標記
ck('meta.version 0.3.3-draft', /version:\s*"0\.3\.3-draft"/.test(t));
...
ck('素材次型五型', has('宣言型') && has('調查型') && has('內部報告型') && has('成功敘事型') && has('法規/理論文本型'));

// 邊界壓測四條回填(v0.3.3)
ck('適用域宣告', has('applicability:') && has('單一運作主體 × 行為證據 × 同一時段'));
ck('純文本不判', has('純文本不判'));
ck('語用檢查', has('語用檢查') && has('擇偶/錄取'));
ck('主體同一性律', has('主體同一性律') && has('對彼此之行為負責或回應'));
ck('爻疑氾濫規則', has('爻疑氾濫規則') && has('主體性存疑'));
