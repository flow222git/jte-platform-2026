#!/usr/bin/env python3
# jte-crypto-testrun.py — jte-crypto.test.html 的可重現測試 runner（只用 Python3 標準函式庫）
# 用法：python3 jte-crypto-testrun.py
#
# 流程：
#   1. 起一個 http server 服務本目錄，並額外接受 POST /__result（把 body 寫到暫存檔、標記完成）；
#   2. 用真實時間背景啟動 headless Chrome 指向測試頁（不用 --virtual-time-budget，
#      實測它會在 WebCrypto async 跑完前就退出）；
...
TEST_PAGE = 'jte-crypto.test.html'
POLL_TIMEOUT = 60.0   # ITER 升到 600000 後測試稍慢，給足輪詢逾時
POLL_INTERVAL = 0.5

result_path = os.path.join(tempfile.gettempdir(), 'jte-crypto-testrun-result.txt')
...
def main():
    ...
    profile_dir = tempfile.mkdtemp(prefix='jte-crypto-chrome-')
    chrome_args = [
        CHROME,
        '--headless=new',
        '--no-sandbox',
